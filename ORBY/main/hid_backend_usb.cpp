#include "hid_adapter.h"
  ...
  static uint16_t mapUsageToConsumerKey(uint16_t usage) {
    switch (usage) {
      case CC_VOL_UP:   return MEDIA_VOLUME_UP;
      case CC_VOL_DOWN: return MEDIA_VOLUME_DOWN;
      case CC_MUTE:     return MEDIA_VOLUME_MUTE;
      default:          return 0;
    }
  }
  void hidConsumerPress(uint16_t usage) {
    uint16_t k = mapUsageToConsumerKey(usage);
    if (k) Consumer.press(k);
  }
  ...
  void hidConsumerClick(uint16_t usage) {
    uint16_t k = mapUsageToConsumerKey(usage);
    if (k) Consumer.write(k);
  }

  // Teclado (HID-Project)
  static void pressMods(uint8_t m){
    if (m & KEY_MOD_LCTRL ) Keyboard.press(KEY_LEFT_CTRL);
    if (m & KEY_MOD_LSHIFT) Keyboard.press(KEY_LEFT_SHIFT);
    if (m & KEY_MOD_LALT  ) Keyboard.press(KEY_LEFT_ALT);
    if (m & KEY_MOD_LGUI  ) Keyboard.press(KEY_LEFT_GUI);
    if (m & KEY_MOD_RCTRL ) Keyboard.press(KEY_RIGHT_CTRL);
    if (m & KEY_MOD_RSHIFT) Keyboard.press(KEY_RIGHT_SHIFT);
    if (m & KEY_MOD_RALT  ) Keyboard.press(KEY_RIGHT_ALT);
    if (m & KEY_MOD_RGUI  ) Keyboard.press(KEY_RIGHT_GUI);
  }
  static void releaseMods(uint8_t m){
    if (m & KEY_MOD_LCTRL ) Keyboard.release(KEY_LEFT_CTRL);
    if (m & KEY_MOD_LSHIFT) Keyboard.release(KEY_LEFT_SHIFT);
    if (m & KEY_MOD_LALT  ) Keyboard.release(KEY_LEFT_ALT);
    if (m & KEY_MOD_LGUI  ) Keyboard.release(KEY_LEFT_GUI);
    if (m & KEY_MOD_RCTRL ) Keyboard.release(KEY_RIGHT_CTRL);
    if (m & KEY_MOD_RSHIFT) Keyboard.release(KEY_RIGHT_SHIFT);
    if (m & KEY_MOD_RALT  ) Keyboard.release(KEY_RIGHT_ALT);
    if (m & KEY_MOD_RGUI  ) Keyboard.release(KEY_RIGHT_GUI);
  }

  void hidKeyboardPress(uint8_t modifiers, uint8_t keycode) {
    if (modifiers) pressMods(modifiers);
    if (keycode != HID_KEY_NONE) Keyboard.press(keycode);
  }
  void hidKeyboardRelease(uint8_t keycode) {
    if (keycode != HID_KEY_NONE) Keyboard.release(keycode);
  }
