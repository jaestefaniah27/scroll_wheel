...
void changeMode(ButtonEvent ev)
{
  ...
  case ButtonEvent::R_LONG: // ZOOM <-> SCROLL  (Ctrl sticky mientras dure el modo)
    ...
    if (wheel_mode == WheelMode::ZOOM)
      hidKeyboardPress(KEY_MOD_LCTRL, HID_KEY_NONE);
    break;

  case ButtonEvent::L_SHORT: // SELECT <-> SCROLL
    wheel_mode = (wheel_mode != WheelMode::SELECT) ? WheelMode::SELECT : WheelMode::SCROLL;
    // if (wheel_mode == WheelMode::SELECT) hidMousePress(MOUSE_LEFT);
    break;
  ...
  case ButtonEvent::RST_LONG: // Reset (mantener 3s)
    // Reset de fábrica o similar
    hidResetConection();
    break;
