...
#define HID_QUEUE 32
    ...
    {
        if (!tud_hid_keyboard_report(REPORT_ID_KEYBOARD, held_mods, held_keys))
            return false;
        sent_mods = held_mods;
    ...
    static void key_up(uint8_t slot)
    {
        if (slot >= HID_SLOTS)
            return;
        if (!slot_mod[slot] && !slot_key[slot])
            return;
        slot_mod[slot] = 0;
        slot_key[slot] = 0;
        recompute();
    ...
    {
        if (slot_mod[HID_SLOT_STICKY] == mod)
            return;
    ...
    static void pump()
    {
        ...
        {
            uint16_t zero = 0;
            if (tud_hid_report(REPORT_ID_CONSUMER, &zero, 2))
                pending_release = 0;
            return;
        }
