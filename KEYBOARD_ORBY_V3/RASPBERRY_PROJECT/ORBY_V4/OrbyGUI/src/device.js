...
export const setScroll     = (v)  => request(`SET_SCROLL:${v}`,     { match: (l) => l.startsWith('SCROLL:OK:') });
export const setScrollInvert = (v) => request(`SET_SCROLL_INV:${v ? 1 : 0}`, { match: (l) => l.startsWith('SCROLL:OK:') });
export const getScroll     = ()   => request('GET_SCROLL',          { match: (l) => l.startsWith('SCROLL:OK:') });
...
// Lee un perfil completo del firmware: nombre, 20 etiquetas, 24 acciones y la
// máscara de iconos personalizados.
export function getProfile(idx) {
  const prof = { idx, name: '', labels: new Array(20).fill(''), keys: [], rotary: [], oledMask: 0 };
  for (let i = 0; i < 24; i++) prof.keys.push({ modifier: 0, keycode: 0 });
  for (let i = 0; i < 8; i++) prof.rotary.push({ type: 0, modifier: 0, keycode: 0 });
